import math
import torch
from torch import nn, Tensor, permute
...
class LearnablePositionalEncoding(nn.Module):
    ...
    def forward(self, X):# x: The sequence fed to the positional encoder model.
        X = X + self.pe[:X.size(0), :]
        return self.dropout(X)


class TransformerBatchEncoderLayer(nn.modules.Module):
    """
    This transformer encoder layer block is made up of self-attn and feedforward network.
    It uses BatchNorm.

    Args: 
         d_model        : the number of expected features in the ínput.
         nhead          : the number of heads in the multiheadattention models.
         dim_feedforward: the dimension of the feedforward network model(default = 2048).
         dropout        : the dropout value (defaul=0.1).
         activation     : the activation function of intermediate layer, relu or gelu (default=relu). 
    """

    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation='relu'):
        super(TransformerBatchEncoderLayer, self).__init__()
...
class TSTransformerEncoder(nn.Module):
    ...
    def __init__(self, feat_dim, max_len, d_model, n_heads, num_layers, dim_feedforward, dropout=0.1,
        ...
        encoder_layer = TransformerBatchEncoderLayer(d_model, self.n_heads, dim_feedforward, dropout*(1.0 - freeze), activation=activation)
